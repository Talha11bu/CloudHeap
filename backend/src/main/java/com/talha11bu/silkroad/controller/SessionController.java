...
public class SessionController {
        ...
        @Operation(summary = "Create a new session", description = "Creates a new file-sharing session with a password and configurable expiration duration. "
                        +
                        "Returns session credentials including a JWT token for the creator.")
        @ApiResponses({
                        @ApiResponse(responseCode = "201", description = "Session created successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = CreateResponse.class), examples = @ExampleObject(value = """
                                        {
                                          "success": true,
        ...
        @Tag(name = "Session Management")
        @Operation(summary = "Join an existing session", description = "Joins an active session using a session ID and password. "
                        +
                        "Returns a JWT token for the user along with the full session state including current users and files.")
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Successfully joined the session", content = @Content(mediaType = "application/json", examples = @ExampleObject(value = """
                                        {
                                          "success": true,
        ...
        @Tag(name = "Session Management")
        @Operation(summary = "Rejoin a session using JWT token", description = "Allows a user to rejoin a session they previously joined by providing their JWT token "
                        +
                        "in the Authorization header. Useful for reconnecting after page refresh or connection drop.", security = @SecurityRequirement(name = "bearerAuth"))
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Successfully rejoined the session", content = @Content(mediaType = "application/json", examples = @ExampleObject(value = """
                                        {
                                          "success": true,
        ...
        public ResponseEntity<JoinResponse> rejoinSession(
                        @Parameter(description = "Bearer JWT token", example = "Bearer eyJhbGciOiJIUzI1NiJ9...", required = true) @RequestHeader("Authorization") String auth) {
        ...
        @Operation(summary = "Get a pre-signed upload URL for a file", description = "Generates a temporary pre-signed URL for uploading a specific file to R2 storage directly from the client. "
                        +
                        "Requires JWT Bearer token authentication.", security = @SecurityRequirement(name = "bearerAuth"))
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Pre-signed URL generated successfully", content = @Content(mediaType = "application/json", examples = @ExampleObject(value = """
                                        {
        ...
        public ResponseEntity<?> getUploadUrl(
                        @Parameter(description = "Session ID", example = "SKR-7X9K2M", required = true) @PathVariable String sessionId,
                        @Parameter(description = "Name of the file", example = "photo.png", required = true) @RequestParam String fileName,
                        @Parameter(description = "Content type of the file", example = "image/png", required = true) @RequestParam String contentType,
                        @Parameter(description = "Bearer JWT token", example = "Bearer eyJhbGciOiJIUzI1NiJ9...", required = true) @RequestHeader(HttpHeaders.AUTHORIZATION) String authHeader) {
                ...
                try {
                        Map<String, String> uploadInfo = sessionService.getPreSignedUploadUrl(sessionId, fileName,
                                        contentType, token);
                        ...
                        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                        .body("Failed to generate upload link");
                }
        }

        @Tag(name = "File Operations")
        @Operation(summary = "Confirm file upload to a session", description = "Confirms that a file has been successfully uploaded to R2. "
                        +
                        "Saves the file metadata and notifies other users in the session. Requires JWT Bearer token authentication.", security = @SecurityRequirement(name = "bearerAuth"))
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "File upload confirmed successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = UploadResponse.class), examples = @ExampleObject(value = """
                                        {
        ...
        public ResponseEntity<?> uploadComplete(
                        @Parameter(description = "ID of the session to upload to", example = "SKR-7X9K2M", required = true) @PathVariable String sessionId,
                        @Parameter(description = "Upload completion details", required = true) @RequestBody UploadCompleteRequest request,
                        @Parameter(description = "Bearer JWT token", example = "Bearer eyJhbGciOiJIUzI1NiJ9...", required = true) @RequestHeader(HttpHeaders.AUTHORIZATION) String authHeader) {
                ...
                try {
                        UploadResponse fileInfo = sessionService.confirmFileUpload(sessionId, request.fileName(),
                                        request.fileKey(), request.fileSize(), token);
        ...
        @Tag(name = "File Operations")
        @Operation(summary = "Download a single file", description = "Downloads a specific file from a session. Requires the session password for authentication. "
                        +
        ...
        public ResponseEntity<Resource> downloadFile(
                        @Parameter(description = "Session ID", example = "SKR-7X9K2M", required = true) @PathVariable String sessionId,
                        @Parameter(description = "Session password", example = "mySecret123", required = true) @RequestParam String password,
                        @Parameter(description = "Name of the file to download", example = "quarterly-report.pdf", required = true) @RequestParam String filename) {
                try {
        ...
        @Tag(name = "File Operations")
        @Operation(summary = "Get a pre-signed download URL for a file", description = "Generates a temporary pre-signed URL for downloading a specific file from R2 storage. "
                        +
                        "Requires JWT Bearer token authentication. The URL is valid for a limited time.", security = @SecurityRequirement(name = "bearerAuth"))
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Pre-signed URL generated successfully", content = @Content(mediaType = "application/json", examples = @ExampleObject(value = """
                                        {
        ...
        public ResponseEntity<?> getFileDownloadUrl(
                        @Parameter(description = "Session ID", example = "SKR-7X9K2M", required = true) @PathVariable String sessionId,
                        @Parameter(description = "Name of the file", example = "photo.png", required = true) @PathVariable String fileName,
                        @Parameter(description = "Bearer JWT token", example = "Bearer eyJhbGciOiJIUzI1NiJ9...", required = true) @RequestHeader(HttpHeaders.AUTHORIZATION) String authHeader) {
                ...
                try {
                        ...
                        if (downloadUrl == null) {
                                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                                .body("Failed to generate download link");
                        }
        ...
        @Tag(name = "File Operations")
        @Operation(summary = "Download all session files as a ZIP", description = "Downloads all files from a session bundled into a single ZIP archive. "
                        +
        ...
        @Tag(name = "Session Management")
        @Operation(summary = "Leave a session", description = "Removes a user from the specified session. Other users in the session will be "
                        +
        ...
        public ResponseEntity<?> leaveSession(
                        @Parameter(description = "Session ID", example = "SKR-7X9K2M", required = true) @PathVariable String sessionId,
                        @Parameter(description = "Username of the user leaving", example = "bob", required = true) @RequestParam String username) {
                try {
        ...
        @Tag(name = "File Operations")
        @Operation(summary = "Delete a file from a session", description = "Deletes a specific file from the session. The file is removed from both the database "
                        +
        ...
        public ResponseEntity<String> deleteFile(
                        @Parameter(description = "Session ID", example = "SKR-7X9K2M", required = true) @PathVariable String sessionId,
                        @Parameter(description = "Name of the file to delete", example = "quarterly-report.pdf", required = true) @RequestParam("fileName") String fileName) {
                try {
        ...
        @Tag(name = "Session Management")
        @Operation(summary = "End a session", description = "Permanently ends a session and deletes all associated files from R2 storage. "
                        +
        ...
        public ResponseEntity<String> endSession(
                        @Parameter(description = "Session ID", example = "SKR-7X9K2M", required = true) @PathVariable String sessionId,
                        @Parameter(description = "Username of the session creator", example = "alice", required = true) @RequestParam String username) {
                try {
